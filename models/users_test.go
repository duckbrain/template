...
func testUserToManySignInAttempts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, signInAttemptDBTypes, false, signInAttemptColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, signInAttemptDBTypes, false, signInAttemptColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.SignInAttempts().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadSignInAttempts(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SignInAttempts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.SignInAttempts = nil
	if err = a.L.LoadSignInAttempts(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SignInAttempts); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpSignInAttempts(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e SignInAttempt

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*SignInAttempt{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, signInAttemptDBTypes, false, strmangle.SetComplement(signInAttemptPrimaryKeyColumns, signInAttemptColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*SignInAttempt{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddSignInAttempts(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.SignInAttempts[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.SignInAttempts[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.SignInAttempts().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `text`, `Username`: `text`, `Provider`: `text`, `PasswordHash`: `text`, `ConfirmedAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
